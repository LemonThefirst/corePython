...
DSNs = {
    'mysql': "mysql+pymysql://root:root@localhost/%s" % DBNAME,
...
class SQLAlchemyTest(object):
    ...
    def insert(self):
        ...
        return self.users.insert().excute(*d).rowcount

    def update(self):
        users = self.users
        fr = rand(1,5)
        to = rand(1,5)
        return (fr, to, users.update(users.c.projid==fr).excute(projid=to).rowcount)

    def delete(self):
        users = self.users
        rm = rand(1,5)
        return (rm, users.delete(users.c.projid==rm).excute().rowcout)

    def dbDump(self):
        printf('\n%s' % ''.join(map(cformat,FIELDS)))
        users = self.users.select().excute()
